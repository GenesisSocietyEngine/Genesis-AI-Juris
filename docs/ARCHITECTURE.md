...
The TypeScript reducer is regression-pinned to 18 distinct mobile reference routes across all five canonical cases and every reachable terminal outcome. Initial state, every command checkpoint, serialization, Rust inspection, load, re-save and final receipts compare exact stage, clock, resources, metrics, evidence, deadlines, inbox state, outcome and judicial-result semantics. Moving this authority into the Rust engine compiled to WASM remains a hardening option once the deployment toolchain can build and verify that artifact; the web runtime does not claim a Rust/WASM binary.
...
v53 separates request and D1-capacity health from product-D1 anomaly storage. Allowlisted `worker.request` and `worker.exception` events go only to the structured Worker/platform log stream; public, asset, anonymous-401 and 404 traffic therefore performs zero telemetry writes in product D1. Routine successful D1/replay/session signals are intentionally not collected. D1 busy/timeout/internal failures are also platform-log only: the signal immediately clears every queued anomaly in the request scope, opens a nonrecursive 60-second process-local circuit, and causes zero telemetry prepare/batch work even in the triggering request. Replay internal failures, historical-version misses, revision/fingerprint mismatches and session internal failures are emitted as structured anomaly logs. Only precise replay/history/revision/fingerprint anomalies from normalized `play_sessions` or `admin` operations are eligible for one bounded D1 batch per request-operation; public catalog signals and session internal failures remain log-only. When no capacity signal occurs, that batch contains one purge of at most 250 expired rows plus no more than two inserts.

A Worker-generated UUID correlates coarse events; raw URLs, query strings, SQL, persistent product IDs, fingerprints, identities and content are excluded. Additive migration `0011` remains justified by the retained anomaly rows and keeps 14-day expiry plus bounded indexes/purge. `/api/admin/operations` and its lazy dashboard stay inside the trusted ChatGPT administrator plus `GENESIS_ADMIN_EMAILS` boundary. The dashboard shows only retained product anomalies and explicitly marks both Worker request health and D1-capacity health as external-notification unavailable when the platform log stream cannot be queried from the Site runtime; missing counts, rates, latency and success data are never fabricated as zero or estimated. Alert policy, the exact six-step request-log configuration, the exact three-step D1-failure-log configuration and rollback actions are versioned in source; see `docs/V53-OBSERVABILITY.md` and `docs/V53-OPERATIONS-RUNBOOK.md`.
