...
#[test]
fn json_protocol_carries_greenfire_production_pressure_through_replay() {
    let mut bridge = MobileBridge::new();
    ...
    let activated_raw = bridge.execute_json(
        &json!({
    ...
    assert_eq!(activated["snapshot"]["clock_minutes"], 120);
    assert_eq!(
        activated["snapshot"]["pressure_and_countermove"],
        json!({
            "projection_schema_version": 1,
    ...
    let opened_raw = bridge.execute_json(
        &json!({
            "command": "dispatch",
            "session_id": original_id,
    ...
    assert_eq!(opened["snapshot"]["clock_minutes"], 180);
    assert_eq!(
    ...
    for (phase, raw) in [
        ...
        for private in [
            ...
            assert!(
                ...
                "{phase} response leaked private pressure internals `{private}`: {raw}"
            );
        }
    }
    ...
    let responded_raw = bridge.execute_json(
        &json!({
            "command": "dispatch",
    ...
    for private in [
        ...
        assert!(
            ...
            "controlled response leaked private pressure internals `{private}`: {responded_raw}"
        );
    }
