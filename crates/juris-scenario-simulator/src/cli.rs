...
use crate::{
    ScenarioDocument, ScenarioSimulator, ScenarioTraceCommand, SimulationError, SimulationResult,
};
...
Usage:\n\
  ...
  juris-scenario-simulator run <scenario.json> --commands-file trace.json [--require-outcome] [--max-auto-events N] [--json]"
}
    ...
    I: Iterator<Item = OsString>,
{
    ...
    if options.values.contains_key("actions") && options.values.contains_key("commands-file") {
        return Err(SimulationError::ConflictingOptions {
            first: "--actions".to_owned(),
            second: "--commands-file".to_owned(),
        });
    }
    ...
    let require_outcome = options.flags.contains("require-outcome");
    let result = match options.values.get("commands-file") {
        Some(path) => {
            let path = PathBuf::from(path);
            let encoded = fs::read_to_string(&path).map_err(|source| SimulationError::Io {
                path: path.clone(),
                source,
            })?;
            let commands: Vec<ScenarioTraceCommand> =
                serde_json::from_str(&encoded).map_err(|source| SimulationError::Json {
                    path: path.clone(),
                    source,
                })?;
            simulator.run_commands(&commands, require_outcome)?
        }
        None => simulator.run_actions(&actions, require_outcome)?,
    };
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    while let Some(raw) = arguments.next() {
        ...
        if name != "actions" && name != "commands-file" && name != "max-auto-events" {
...
fn render_text(result: &SimulationResult) -> String {
    ...
    format!(
        ...
        serde_json::to_string(&result.deadline_statuses).unwrap_or_else(|_| "{}".to_owned()),
        serde_json::to_string(&result.async_task_statuses).unwrap_or_else(|_| "{}".to_owned())
    )
}
