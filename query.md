# Запрос на архитектурное решение: миграция Failed ERP в authoritative Rust runtime

## Назначение документа

Этот документ предназначен для передачи в ChatGPT как самостоятельный архитектурный запрос. Нужны конкретные решения по противоречиям текущего legacy-сценария Failed ERP до начала реализации. Нельзя предлагать приблизительную миграцию или молча выбирать новую семантику.
...
До получения этих решений implementation на ветке `refactor/failed-erp-authoritative-rust` должен оставаться остановленным.
