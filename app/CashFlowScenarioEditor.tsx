...
export default function CashFlowScenarioEditor({ locale, model, beginFieldEdit, commitField, setModel, changeRepaymentBasis }: {
  ...
  return <fieldset className="cash-flow-scenario-fields"><legend>{locale === "en" ? "CASH-FLOW SCENARIO" : "CASH-FLOW СЦЕНАРИЙ"}</legend>
    <p>{locale === "en" ? "This node controls the case economics. The four probability bands above recalculate from these inputs; extra cash-flow nodes are for separate payment streams, not probability bands." : "Эта нода управляет экономикой кейса. Четыре вероятностных диапазона выше пересчитываются из этих параметров; дополнительные cash-flow ноды нужны для отдельных потоков платежей, а не для диапазонов вероятности."}</p>
