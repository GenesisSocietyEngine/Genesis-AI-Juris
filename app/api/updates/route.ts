...
export async function GET() {
  ...
  const [[profile], subscriptions, reads, published] = await Promise.all([
    ...
    db.select().from(updates).where(and(isNotNull(updates.publishedAt), or(eq(updates.expiresAt, ""), isNotNull(updates.expiresAt)))).orderBy(desc(updates.publishedAt)).limit(50),
  ]);
