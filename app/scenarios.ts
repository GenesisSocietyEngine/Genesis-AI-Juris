...
type CanonicalAction = {
  id: string; title: string; description: string; effects: CanonicalEffect[];
  time_cost_minutes?: number; cost_eur?: number;
  completion_timing?: { calendar_target?: { day_offset: number; minute_of_day: number } };
  repeatability?: { type?: string; max_uses?: number };
};
type CanonicalStage = { id: string; title: string; kind: string; exit_actions: string[]; entry_event?: string; terminal?: boolean };
type CanonicalEvent = {
  id: string;
  trigger?: { type?: string; action?: string; deadline?: string };
  effects?: CanonicalEffect[];
};
type CanonicalDeterministicDecision = {
  id: string;
  branches?: Array<{ effects?: CanonicalEffect[] }>;
};
...
type CanonicalCase = {
  ...
  scenario: {
    ...
    deterministic_decisions?: CanonicalDeterministicDecision[];
...
const baseScenarios: BaseScenario[] = [
  ...
  },
  {
    ...
    jurisdiction: "BE · Commercial",
    role: t("Юрист кредитора", "Creditor counsel"), version: "1.0.0",
  ...
  },
  {
    ...
    jurisdiction: "NL · Corporate / Regulatory", role: t("Управляющий директор", "Managing Director"), version: "0.2.0", sector: t("Промышленность / кризис", "Industrial / crisis"), urgency: "critical", fingerprint: "173140f0…bde2438", accent: "#f06b4f",
  ...
  },
  {
    ...
    subtitle: t("Загрязнение цепочки поставок яиц", "Contaminated egg supply chain"), jurisdiction: "NL · Food safety", role: t("Координатор претензий", "Claims coordinator"), version: "0.1.0", sector: t("Пищевая безопасность", "Food safety"), urgency: "critical", fingerprint: "7b0d2d7f…d18ba4", accent: "#f0c35b",
  ...
  },
  {
    ...
    order: 50,
    title: t("Вода пустыни", "Desert Water"), subtitle: t("Подземные воды, общий источник и цена доказательства", "Groundwater, a shared source and the price of proof"), jurisdiction: "US · Environmental", role: t("Ведущий юрист истцов", "Lead claimant counsel"), version: "0.1.0", sector: t("Экология / массовые требования", "Environmental / mass claims"), urgency: "elevated", fingerprint: "636e7b78…c41a28af", accent: "#58b6a7",
...
function canonicalMetricEffects(effects: CanonicalEffect[]): Partial<Record<MetricKey, number>> {
  ...
  const add = (key: MetricKey, amount: number) => { result[key] = (result[key] ?? 0) + amount; };
  const metricMap: Record<string, MetricKey | undefined> = {
    merits: "position", procedure: "position", case_strength: "position", leverage: "position",
    evidence: "evidence", client_trust: "trust", ethics: "trust",
    fatigue: "exposure", cumulative_strain: "exposure",
  };
  for (const effect of effects) {
    ...
    const amount = Math.max(-24, Math.min(24, effect.amount * direction));
    add(key, key === "exposure" ? amount : amount);
  }
  return result;
}
...
function effectsNextStage(raw: CanonicalCase["scenario"], effects: CanonicalEffect[], visited: Set<string>): string | undefined {
  ...
  for (const effect of effects) {
    ...
    if (effect.type === "resolve_deterministic_decision" && effect.decision) {
      const nested = decisionNextStage(raw, effect.decision, visited);
      if (nested) return nested;
    }
  }
  return undefined;
...
function decisionNextStage(raw: CanonicalCase["scenario"], decisionId: string, visited: Set<string>): string | undefined {
  const token = `decision:${decisionId}`;
  if (visited.has(token)) return undefined;
  visited.add(token);
  const decision = raw.deterministic_decisions?.find((item) => item.id === decisionId);
...
function eventNextStage(raw: CanonicalCase["scenario"], eventId: string, visited = new Set<string>()): string | undefined {
  ...
  return effectsNextStage(raw, event.effects ?? [], visited);
}

function actionNextStage(raw: CanonicalCase["scenario"], action: CanonicalAction): string | undefined {
  const direct = effectsNextStage(raw, action.effects, new Set<string>());
  if (direct) return direct;
  for (const event of raw.events ?? []) {
    if (event.trigger?.type !== "after_action" || event.trigger.action !== action.id) continue;
    const triggered = eventNextStage(raw, event.id);
    if (triggered) return triggered;
  }
  return undefined;
}

function adaptCanonicalStage(canonicalCase: CanonicalCase, rawStage: CanonicalStage): ScenarioStage {
  ...
  const options = rawStage.exit_actions.flatMap((id): DecisionOption[] => {
    ...
    if (!action) return [];
    const nextStageId = actionNextStage(raw, action);
    const calendarTarget = action.completion_timing?.calendar_target;
    const repeatability = action.repeatability?.type === "once" ? "once" : action.repeatability?.type === "limited" ? "limited" : "repeatable";
    return [{
      id: action.id,
      ...
      result: {
        en: nextStageId ? `Action recorded. The matter advances to ${raw.stages.find((stage) => stage.id === nextStageId)?.title ?? nextStageId}.` : "Action recorded in the live matter. Review the updated record before continuing.",
        ru: nextStageId ? `Действие зарегистрировано. Дело переходит на стадию «${localizedText(canonicalCase, "stages", nextStageId, "title", raw.stages.find((stage) => stage.id === nextStageId)?.title ?? nextStageId).ru}».` : "Действие зарегистрировано в живом деле. Перед продолжением проверьте обновлённую запись.",
      },
      cost: action.cost_eur ?? 0,
      minutes: action.time_cost_minutes ?? 0,
      effects: canonicalMetricEffects(action.effects),
      nextStageId,
  ...
  const stageTitle = localizedText(canonicalCase, "stages", rawStage.id, "title", rawStage.title);
  return {
    id: rawStage.id,
    day: 1,
    time: "00:00",
    ...
    headline: stageTitle,
    brief: {
      en: rawStage.id === raw.initial_stage ? raw.metadata.summary : `Complete the available ${rawStage.kind.replaceAll("_", " ")} work. Each confirmed action advances the authoritative foreground clock.`,
      ru: rawStage.id === raw.initial_stage ? (canonicalCase.scenario_localizations?.ru?.metadata?.summary ?? raw.metadata.summary) : `Выполните доступные действия стадии «${stageTitle.ru}». Каждое подтверждённое действие продвигает игровое время.`,
    },
    source: { en: `Canonical workflow · ${rawStage.kind}`, ru: `Канонический процесс · ${rawStage.kind}` },
    materialRefs: [],
    options,
...
function adaptDeadlines(canonicalCase: CanonicalCase): ScenarioDeadline[] {
  const raw = canonicalCase.scenario;
  return raw.deadlines.map((deadline) => ({
    id: deadline.id,
    ...
    completionActions: deadline.completion_actions,
    activationEvent: deadline.activation_event,
    missedNextStageId: deadline.missed_event ? eventNextStage(raw, deadline.missed_event) : undefined,
  }));
}
...
function adaptInbox(canonicalCase: CanonicalCase): ScenarioInboxItem[] {
  return canonicalCase.scenario.inbox_items.map((item) => ({
    ...
    resolutionActions: item.resolution_actions ?? [],
  }));
}
...
export const scenarios: Scenario[] = baseScenarios.map((base) => {
  ...
  return {
    ...
    initialClockMinute: (raw.initial_clock?.day ?? 0) * 1440 + (raw.initial_clock?.minute_of_day ?? 0),
    stages: raw.stages.map((stage) => adaptCanonicalStage(canonicalCase, stage)),
    deadlines: adaptDeadlines(canonicalCase),
    workflowInbox: adaptInbox(canonicalCase),
  };
});
