...
export default function CaseMarkdownActions({locale,loadDisabled,exportDisabled,loaded,opened,failed}:{locale:"en"|"ru";loadDisabled:boolean;exportDisabled:boolean;loaded:(value:string)=>void;opened:()=>void;failed:(message:string)=>void}){
  ...
  return <>
    <button className="secondary-cta" onClick={()=>input.current?.click()} disabled={loadDisabled}>{locale==="en"?"Load case MD":"Загрузить MD кейса"}</button>
    <button className="secondary-cta" onClick={()=>void open()} disabled={exportDisabled}>{locale==="en"?"Polished case MD":"Описание кейса MD"}</button>
